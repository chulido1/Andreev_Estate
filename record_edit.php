...
<head>
    <meta charset="UTF-8" />
    <title>Редактировать запись в <?= htmlspecialchars($table) ?></title>
    <style>
        ...
        a { display: inline-block; margin-top: 15px; text-decoration: none; color: black; }
    </style>
</head>
<body>
    <h1>Редактировать запись в таблице <?= htmlspecialchars($table) ?></h1>
    <form method="POST">
            ...
            <?php if (isset($options[$field])): ?>
                <select name="<?= htmlspecialchars($field) ?>" id="<?= htmlspecialchars($field) ?>" required>
                    ...
                    <?php foreach ($options[$field] as $opt): ?>
                        <option
                            value="<?= htmlspecialchars($opt[$foreignKeys[$table][$field]['id']]) ?>"
                            <?= ($record[$field] == $opt[$foreignKeys[$table][$field]['id']]) ? 'selected' : '' ?>
                        >
